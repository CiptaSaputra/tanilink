      ...
      <div className="bg-white border border-nat-border rounded-2xl p-5 shadow-sm flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-xs text-nat-sage mt-0.5">
            Pemantauan performa Smart Matching, resolusi dispute, dan status
            distribusi.
          </p>
        </div>
        <button
      ...
      <div className="flex border-b border-nat-border gap-2">
        <button
          ...
          className={`px-4 py-2 text-xs font-bold transition-all border-b-2 flex items-center gap-1.5 cursor-pointer ${
            activeTab === "matching"
              ? "border-nat-green text-nat-green-hover"
        ...
        <div className="space-y-6">
          ...
          >
            <div className="flex items-start gap-2 mb-4 pb-2 border-b border-nat-light-cream">
              ...
              <div>
                <h3 className="text-sm font-bold text-nat-dark">
                  Bobot Default Smart Matching per Komoditas
                </h3>
                <p className="text-[10px] text-nat-sage">
                  Bobot default sudah ditetapkan per kategori komoditas (PRD).
                  Admin tidak mengatur bobot secara bebas.
                </p>
              </div>
            </div>
              ...
              <table className="w-full text-left text-xs">
                ...
                <tbody>
                  {Object.entries(COMMODITY_WEIGHTS).map(([crop, w]) => {
                    const meta =
                      COMMODITY_LIST[crop as keyof typeof COMMODITY_LIST];
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="py-3 text-center font-bold text-nat-dark">
                          {(w.wLocation * 100).toFixed(0)}%
                        </td>
                        <td className="py-3 text-center font-bold text-nat-dark">
                          {(w.wVolume * 100).toFixed(0)}%
                        </td>
                        <td className="py-3 text-center font-bold text-nat-dark">
                          {(w.wPrice * 100).toFixed(0)}%
                        </td>
                      ...
                      <div className="flex justify-between items-start mb-2 pb-2 border-b border-nat-border/40">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="px-1.5 py-0.5 rounded text-[8px] font-bold border bg-nat-light-cream text-nat-sage">
                            Bobot:{" "}
                            {COMMODITY_WEIGHTS[harvest.commodity]
                              ? `${(COMMODITY_WEIGHTS[harvest.commodity].wLocation * 100).toFixed(0)}/${(COMMODITY_WEIGHTS[harvest.commodity].wVolume * 100).toFixed(0)}/${(COMMODITY_WEIGHTS[harvest.commodity].wPrice * 100).toFixed(0)}`
                              : "-"}
                          </span>
                        </div>
                        <div className="flex items-center gap-2">
                      ...
                      <div className="grid grid-cols-2 gap-2 text-[10px] text-nat-sage mb-2">
                        <div>
                          <span className="block font-bold text-nat-dark flex items-center gap-0.5">
                            <User className="w-3 h-3" /> {harvest.farmerName} (
                            {harvest.region})
                          </span>
                          {harvest.expectedVolume.toLocaleString("id-ID")} Kg @
                          Rp{harvest.askingPrice.toLocaleString("id-ID")}
                        </div>
                        <div>
                          <span className="block font-bold text-nat-dark flex items-center gap-0.5">
                            <User className="w-3 h-3" /> {demand.buyerName} (
                            {demand.region})
                          </span>
                          {demand.requiredVolume.toLocaleString("id-ID")} Kg @
                          Rp{demand.offerPrice.toLocaleString("id-ID")}
                        </div>
                      </div>
            ...
            >
              ...
              <div className="space-y-2">
                {reviews.map((r) => (
                  <div
                    key={r.id}
                    className="flex items-center justify-between p-3 bg-nat-light-cream/50 border border-nat-border rounded-xl text-xs"
                  >
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-nat-sage ml-2">
                        {r.comment || "—"}
                      </span>
                    </div>
                    <span className="text-[9px] text-nat-sage">
                      Pre-Order: {r.preOrderId.slice(0, 12)}
                    </span>
                  </div>
                ))}
              </div>
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            <div className="bg-white rounded-2xl border border-nat-border p-4 shadow-sm">
              ...
              <p className="text-2xl font-bold text-nat-dark mt-1">
            ...
            <div className="bg-white rounded-2xl border border-nat-border p-4 shadow-sm">
              ...
              <p className="text-2xl font-bold text-nat-dark mt-1">
                {harvestBatches.filter((b) => b.status === "IN_TRANSIT").length}
              </p>
            </div>
            <div className="bg-white rounded-2xl border border-nat-border p-4 shadow-sm">
              <p className="text-[10px] text-nat-sage uppercase tracking-wider font-bold flex items-center gap-1">
              ...
              <p className="text-2xl font-bold text-nat-dark mt-1">
                {harvestBatches.filter((b) => b.status === "DELIVERED").length}
              </p>
            </div>
            <div className="bg-white rounded-2xl border border-nat-border p-4 shadow-sm">
              <p className="text-[10px] text-nat-sage uppercase tracking-wider font-bold flex items-center gap-1">
              ...
              <p className="text-2xl font-bold text-nat-dark mt-1">
                {preOrders.filter((po) => po.status === "CONFIRMED").length}
              </p>
            </div>
          </div>
                    ...
                    return (
                      ...
                      >
                        <div className="flex justify-between items-center mb-2">
                          ...
                          <div
                            className={`font-black text-sm ${batch.priorityScore >= 70 ? "text-red-600" : batch.priorityScore >= 40 ? "text-nat-brown" : "text-nat-green"}`}
                          >
                        ...
                        <div className="w-full bg-nat-border h-1.5 rounded-full mb-2 overflow-hidden">
                          <div
                            className={`h-full rounded-full ${batch.priorityScore >= 70 ? "bg-red-500" : batch.priorityScore >= 40 ? "bg-nat-brown" : "bg-nat-green"}`}
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-[10px] text-nat-sage mb-2">
                          ...
                          <div>
                            <span className="block">Umur Simpan</span>
                            <span className="font-bold">
                              {batch.shelfLifeDays} hari
                            </span>
                          </div>
                          <div>
                            <span className="block">Tgl Panen</span>
                            <span className="font-bold">
                              {batch.harvestDate}
                            </span>
                          </div>
                        ...
                        <div className="flex justify-between pt-2 border-t border-nat-border">
                          <span
                            className={`px-2 py-0.5 rounded text-[9px] font-bold border ${
                              batch.status === "READY"
                                ? "bg-nat-light-cream text-nat-brown border-nat-border"
                                : batch.status === "IN_TRANSIT"
                                  ? "bg-nat-light-cream text-nat-dark border-nat-border"
                                  : batch.status === "PICKED_UP_DIRECTLY"
                          ...
                          <div className="flex gap-1.5">
                            {batch.status === "READY" && (
                              <button
                                onClick={() =>
                                  updateBatchStatus(batch.id, "IN_TRANSIT")
                                }
                    ...
                    >
                      <div className="flex items-center gap-2">
                        <span
                          ...
                          style={{
                            backgroundColor:
                              COMMODITY_LIST[po.commodity]?.color,
                          }}
                        />
                        <div>
                          <span className="font-bold text-nat-dark">
                          ...
                          <span className="text-nat-sage ml-2">
                            • {po.farmerName} → {po.buyerName}
                          </span>
                          <span className="text-[9px] text-nat-sage ml-2">
                            |{" "}
                            {po.deliveryMode === "direct"
                              ? "Langsung"
                              : "Konsolidasi"}
                      ...
                      <div className="text-right text-[10px]">
                        ...
                        <div className="font-bold text-nat-green-hover">
                          Rp{po.agreedPricePerKg.toLocaleString("id-ID")}/Kg
                        </div>
                      </div>
                    </div>
                  );
