...
export default function PPLView() {
  ...
  const matchedCount = useMemo(
    () =>
      regionalHarvests.filter(
        (h) => h.status === "MATCHED" || h.status === "HARVESTED",
      ).length,
    [regionalHarvests],
  );
  ...
  const commodityBreakdown = useMemo(() => {
    const breakdown: Record<
      string,
      { totalVolume: number; count: number; matched: number }
    > = {};
    regionalHarvests.forEach((h) => {
      const key = h.commodity;
      if (!breakdown[key])
        breakdown[key] = { totalVolume: 0, count: 0, matched: 0 };
      breakdown[key].totalVolume += h.expectedVolume;
      breakdown[key].count += 1;
      if (h.status === "MATCHED" || h.status === "HARVESTED")
        breakdown[key].matched += 1;
    });
    return breakdown;
  }, [regionalHarvests]);
    ...
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-teal-700 to-teal-500 rounded-2xl p-6 text-white shadow-sm flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <h2 className="text-xl font-bold tracking-tight">
            Selamat Datang, {activeUser.PPL.name}
          </h2>
      ...
      <div className="bg-teal-50 border border-teal-200/60 rounded-xl p-4 flex gap-3 text-xs text-teal-900">
        ...
        <div>
          ...
          <p className="mt-1 text-teal-800 leading-relaxed">
            ...
            <span className="font-semibold">read-only</span> sesuai PRD
            TaniLink. PPL/BPP dapat memantau data agregat wilayah binaan:
            planting yang dipublikasikan, estimasi panen, dan status
            konsolidasi. Tidak ada mekanisme input atau perubahan data planting
            di sini. Pendampingan petani dilakukan di luar sistem secara
            personal.
          </p>
        </div>
      </div>
      ...
      <div id="ringkasan" className="grid grid-cols-2 sm:grid-cols-4 gap-4 scroll-mt-28">
        ...
        <div className="bg-white rounded-2xl border border-nat-border p-4 shadow-sm">
          ...
          <p className="text-xl font-bold text-nat-dark mt-1">
            {regionalBatches.filter((b) => b.status === "READY").length}
          </p>
        </div>
        <div className="bg-white rounded-2xl border border-nat-border p-4 shadow-sm">
          <p className="text-[10px] text-nat-sage uppercase tracking-wider font-bold flex items-center gap-1">
        ...
        {Object.keys(commodityBreakdown).length > 0 ? (
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs text-nat-text">
              ...
              <tbody>
                {Object.entries(commodityBreakdown).map(
                  ([crop, data]: [
                    string,
                    { totalVolume: number; count: number; matched: number },
                  ]) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="py-3 font-bold">
                          {data.totalVolume.toLocaleString("id-ID")} Kg
                        </td>
                        ...
                        <td className="py-3">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-[10px] font-bold">
                              {data.count > 0
                                ? Math.round((data.matched / data.count) * 100)
                                : 0}
                              %
                            </span>
                          </div>
          ...
          <div className="text-center py-6 text-nat-sage italic">
            Belum ada data tanam yang dipublikasikan di wilayah ini.
          </div>
        )}
      </div>
      ...
      >
        ...
        {regionalHarvests.length > 0 ? (
          ...
          <div className="text-center py-6 text-nat-sage italic">
            Belum ada data tanam dipublikasikan di wilayah ini.
          </div>
        )}
      </div>
