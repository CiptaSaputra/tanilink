...
type RegisterableRole = Extract<Role, 'PETANI' | 'PEMBELI' | 'PPL' | 'KOLEKTOR'>;

const REGISTERABLE_ROLES: { value: RegisterableRole; label: string; desc: string }[] = [
  { value: 'PETANI',   label: 'Petani',                 desc: 'Input data tanam, kelola hasil panen, pre-order' },
  { value: 'PEMBELI',  label: 'Pembeli / Koperasi',     desc: 'Publish permintaan, matching, pre-order' },
  { value: 'PPL',      label: 'PPL / BPP',              desc: 'Monitoring wilayah binaan (read-only)' },
  { value: 'KOLEKTOR', label: 'Kolektor',               desc: 'Lihat rute rekomendasi, update status batch' },
];

const REGIONS = [
  'Brebes', 'Garut', 'Malang', 'Magelang', 'Semarang', 'Cirebon',
  'Bandung', 'Yogyakarta', 'Surabaya', 'Jakarta', 'Medan', 'Makassar',
  'Jawa Tengah', 'Jawa Barat', 'Jawa Timur', 'Sumatera Utara',
];
    ...
    <div className="min-h-screen bg-nat-light-cream/50 flex items-center justify-center px-4 py-12">
      ...
      >
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-bold text-nat-dark">Daftar ke TaniLink</h1>
          <p className="text-sm text-nat-sage mt-1">Buat akun untuk mulai menggunakan platform</p>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-nat-border p-8">
          <form onSubmit={handleSubmit} className="space-y-4" noValidate>
            ...
            <div className="space-y-1.5">
              <label htmlFor="name" className="block text-sm font-medium text-nat-dark">Nama Lengkap</label>
            ...
            {/* Email */}
            <div className="space-y-1.5">
              <label htmlFor="email" className="block text-sm font-medium text-nat-dark">Email</label>
              <div className="relative">
            ...
            {/* Role */}
            <div className="space-y-1.5">
              <label htmlFor="role" className="block text-sm font-medium text-nat-dark">Peran</label>
              <div className="relative">
            ...
            {/* Wilayah */}
            <div className="space-y-1.5">
              <label htmlFor="region" className="block text-sm font-medium text-nat-dark">Wilayah / Kabupaten</label>
              <div className="relative">
              ...
              {fieldErrors.region && <p className="text-xs text-red-600">{fieldErrors.region}</p>}
            </div>

            {/* Password */}
            <div className="space-y-1.5">
              <label htmlFor="password" className="block text-sm font-medium text-nat-dark">Password</label>
              <div className="relative">
                ...
                />
                <button type="button" onClick={() => setShowPassword(v => !v)} className="absolute right-3 top-1/2 -translate-y-1/2 text-nat-sage hover:text-nat-dark transition-colors cursor-pointer" aria-label={showPassword ? 'Sembunyikan' : 'Tampilkan'}>
            ...
            {/* Konfirmasi Password */}
            <div className="space-y-1.5">
              <label htmlFor="confirmPassword" className="block text-sm font-medium text-nat-dark">Konfirmasi Password</label>
              <div className="relative">
                <Lock className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-nat-sage pointer-events-none" />
                <input
                  id="confirmPassword"
                ...
                />
                <button type="button" onClick={() => setShowConfirm(v => !v)} className="absolute right-3 top-1/2 -translate-y-1/2 text-nat-sage hover:text-nat-dark transition-colors cursor-pointer" aria-label={showConfirm ? 'Sembunyikan' : 'Tampilkan'}>
            ...
            <p className="text-[11px] text-nat-sage bg-nat-light-cream rounded-lg px-3 py-2">
              ℹ️ Akun <strong>Admin</strong> dan <strong>Dinas Pertanian</strong> tidak tersedia untuk pendaftaran mandiri.
            </p>
