...
export default function KolektorView() {
  ...
  const regionalBatches = useMemo(() =>
    harvestBatches.filter(
      b => b.region.toLowerCase() === activeUser.KOLEKTOR.region.toLowerCase() &&
           b.status === 'READY'
    ),
  ...
  const optimizedRoutes = useMemo(() => {
    ...
    const depot = depotCoords[activeUser.KOLEKTOR.region] || { lat: -6.871, lng: 109.042 };
    ...
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-amber-700 to-amber-500 rounded-2xl p-6 text-white shadow-sm flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <h2 className="text-xl font-bold tracking-tight">{activeUser.KOLEKTOR.name}</h2>
          <p className="text-xs text-amber-100 mt-1">
            Wilayah: <span className="font-semibold text-white">{activeUser.KOLEKTOR.region}</span> | Rute bersifat rekomendasi
          </p>
        </div>
        <div className="flex gap-4">
          <div className="bg-white/10 backdrop-blur-sm px-4 py-2 rounded-xl border border-white/10">
            <p className="text-[10px] text-amber-100 uppercase tracking-wider font-semibold">Siap Jemput</p>
            <p className="text-lg font-bold">{regionalBatches.length} Batch</p>
          </div>
          <div className="bg-white/10 backdrop-blur-sm px-4 py-2 rounded-xl border border-white/10">
            <p className="text-[10px] text-amber-100 uppercase tracking-wider font-semibold">Sedang Dikirim</p>
            <p className="text-lg font-bold text-amber-200">
              {allRegionalBatches.filter(b => b.status === 'IN_TRANSIT' || b.status === 'PICKED_UP_DIRECTLY').length}
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-amber-50 border border-amber-200/60 rounded-xl p-4 flex gap-3 text-xs text-amber-900">
        ...
        <div>
          ...
          <p className="mt-1 text-amber-800 leading-relaxed">
            Urutan penjemputan yang ditampilkan adalah <span className="font-semibold">saran sistem</span> berdasarkan jarak terpendek. Anda tetap bebas menentukan urutan sendiri sesuai kondisi lapangan. Gunakan tombol "Jemput Langsung" jika pembeli mengambil sendiri tanpa melalui titik kumpul.
          </p>
        </div>
      </div>

      {/* Route Recommendation */}
      {optimizedRoutes.length > 0 && optimizedRoutes[selectedRoute]?.routeStops.length > 0 ? (
          ...
          <div className="bg-white rounded-2xl border border-nat-border overflow-hidden shadow-sm">
            <div className="bg-slate-900 text-white p-4 flex justify-between items-center">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <h4 className="font-bold text-xs">{optimizedRoutes[selectedRoute].vehicleName}</h4>
                  <p className="text-[9px] text-slate-400">Kapasitas: {optimizedRoutes[selectedRoute].capacityKg.toLocaleString('id-ID')} Kg</p>
                </div>
              </div>
              <div className="text-right">
                <p className="text-[10px] text-slate-400">Jarak: {optimizedRoutes[selectedRoute].totalDistanceKm} Km</p>
              </div>
            </div>

            <div className="p-4 space-y-4">
              <p className="text-[10px] font-bold text-nat-sage uppercase tracking-wider">Urutan Rekomendasi Penjemputan</p>
              <div className="space-y-3 relative before:absolute before:left-3 before:top-2 before:bottom-2 before:w-[1.5px] before:bg-amber-100">
                {optimizedRoutes[selectedRoute].routeStops.map((stop, idx) => (
                  <div key={stop.harvestId} className="flex gap-3 items-start relative pl-6">
                    ...
                    <div className="space-y-0.5 text-xs">
                      ...
                      <p className="text-[10px] text-nat-text">{stop.commodity} • {stop.volumeKg.toLocaleString('id-ID')} Kg</p>
                      <p className="text-[9px] text-nat-sage flex items-center gap-0.5">
                        <MapPin className="w-2.5 h-2.5" /> Lat: {stop.latitude.toFixed(3)}, Lng: {stop.longitude.toFixed(3)}
                      </p>
                    </div>
                  </div>
                ))}
                ...
                <div className="flex gap-3 items-start relative pl-6">
                  ...
                  <div className="text-xs space-y-0.5">
                    ...
                    <p className="text-[9px] text-nat-sage">Selesai bongkar muatan</p>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="bg-white rounded-2xl border border-nat-border p-5 shadow-sm">
            ...
            <div className="space-y-3">
              {regionalBatches.map(b => {
                ...
                return (
                  <div key={b.id} className="border border-nat-border rounded-xl p-3.5 bg-nat-light-cream/50 text-xs">
                    <div className="flex justify-between items-start mb-2">
                      ...
                      <span className={`px-2 py-0.5 rounded text-[9px] font-bold ${
                        b.priorityScore >= 70 ? 'bg-red-100 text-red-700'
                        : b.priorityScore >= 40 ? 'bg-amber-100 text-amber-700'
                    ...
                    <div className="grid grid-cols-2 gap-2 text-[10px] text-nat-sage mb-2">
                      <span>Volume: <span className="font-bold text-nat-dark">{b.actualVolumeKg.toLocaleString('id-ID')} Kg</span></span>
                      <span>Umur Simpan: <span className="font-bold">{b.shelfLifeDays} hari</span></span>
                    </div>
                    <div className="flex gap-2">
                      <button
                      ...
                      <button
                        onClick={() => updateBatchStatus(b.id, 'PICKED_UP_DIRECTLY')}
        ...
        <div className="bg-white border border-nat-border rounded-2xl p-8 text-center max-w-lg mx-auto space-y-3 shadow-sm">
          ...
          <h4 className="font-bold text-sm text-nat-dark">Tidak Ada Batch Siap Jemput</h4>
          <p className="text-xs text-nat-sage leading-relaxed">
            Belum ada batch panen yang siap dijemput di wilayah {activeUser.KOLEKTOR.region}.
      ...
      {allRegionalBatches.filter(b => b.status === 'IN_TRANSIT' || b.status === 'DELIVERED').length > 0 && (
        <div className="bg-white rounded-2xl border border-nat-border p-5 shadow-sm">
          <h3 className="text-sm font-bold text-nat-dark mb-4 pb-2 border-b border-nat-light-cream flex items-center gap-1.5">
          ...
          <div className="space-y-2">
            {allRegionalBatches.filter(b => b.status !== 'READY').map(b => (
              <div key={b.id} className="flex items-center justify-between p-3 bg-nat-light-cream/40 border border-nat-border rounded-xl text-xs">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-bold text-nat-dark">{b.farmerName}</span>
                  <span className="text-nat-sage">• {b.commodity} • {b.actualVolumeKg.toLocaleString('id-ID')} Kg</span>
                </div>
                <div className="flex items-center gap-2">
                  <span className={`px-2 py-0.5 rounded text-[9px] font-bold border ${
                    ...
                    {b.status === 'IN_TRANSIT' ? 'Dalam Perjalanan' : b.status === 'PICKED_UP_DIRECTLY' ? 'Jemput Langsung' : 'Terkirim'}
                  </span>
                  {b.status === 'IN_TRANSIT' && (
                    <button
