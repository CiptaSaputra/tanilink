...
export default function BuyerView({ mapLat, mapLng, mapRegion, clearMapSelection }: BuyerViewProps) {
  ...
  const handleSimulatedScan = () => {
    ...
    setTimeout(() => {
      ...
      if (matchedHarvest) {
        showNotification(`SCAN BERHASIL: Batch ${matchedHarvest.commodity} #${matchedHarvest.id} terverifikasi asli di Sistem!`, 'success');
          ...
          <div className="bg-white rounded-2xl border border-nat-border p-5 shadow-sm space-y-4">
            <div>
              ...
              <p className="text-[11px] text-nat-sage mt-1 font-medium">
                Pindai QR Code fisik komoditas panen untuk membaca data digital di Sistem Food Loss.
              </p>
            </div>
