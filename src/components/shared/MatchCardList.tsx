      ...
      <div className="flex justify-between items-center mb-4 pb-2 border-b border-nat-light-cream">
        <h3 className="text-sm font-bold text-nat-dark flex items-center gap-1.5">
          ...
          {userRole === "PETANI"
            ...
            : "Rekomendasi Petani Terdekat (Skor Match Cerdas)"}
        </h3>
      ...
      {matches.length > 0 ? (
        <div className="space-y-4">
          {matches.map((match) => {
            ...
            const peerName =
              userRole === "PETANI" ? demand.buyerName : harvest.farmerName;
            const peerId =
              userRole === "PETANI" ? demand.buyerId : harvest.farmerId;
            const peerRegion =
              userRole === "PETANI" ? demand.region : harvest.region;
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 bg-nat-light-cream p-2.5 rounded-lg text-[11px] mb-3 text-nat-text border border-nat-border">
                  ...
                  <div>
                    ...
                    <p className="font-bold text-nat-dark mt-0.5">
                      Rp
                      {Math.abs(
                        demand.offerPrice - harvest.askingPrice,
                    ...
                    <div className="w-full bg-nat-cream h-1 rounded-full mt-1 overflow-hidden">
                      <div
                        ...
                        style={{ width: `${match.scoreDetails.priceScore}%` }}
                      />
                    </div>
                  </div>
                  {/* 3. Kesesuaian Volume */}
                  <div>
                    ...
                    <div className="w-full bg-nat-cream h-1 rounded-full mt-1 overflow-hidden">
                      <div
                        className="bg-nat-brown h-full rounded-full"
                        style={{ width: `${match.scoreDetails.volumeScore}%` }}
                      />
                    </div>
                  </div>
                </div>
