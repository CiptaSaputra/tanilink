...
export const MyHarvestsTable: React.FC<MyHarvestsTableProps> = ({ harvests, onOpenHarvestModal }) => {
  return (
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="border-b border-nat-border/50 text-[10px] uppercase text-nat-sage">
              <th className="py-3 px-2 font-bold whitespace-nowrap">Komoditas / ID</th>
              <th className="py-3 px-2 font-bold whitespace-nowrap">Tgl Tanam & Panen</th>
              <th className="py-3 px-2 font-bold whitespace-nowrap">Estimasi & Harga</th>
              <th className="py-3 px-2 font-bold whitespace-nowrap">Status Lahan</th>
              <th className="py-3 px-2 font-bold whitespace-nowrap text-right">Aksi Distribusi</th>
            </tr>
          </thead>
            ...
            {harvests.length > 0 ? (
              harvests.map((h) => {
                ...
                const isNearingHarvest =
                  harvestDate.getTime() - today.getTime() < 14 * 24 * 60 * 60 * 1000 &&
                  h.status === 'ACTIVE';

                return (
                  <tr key={h.id} className="border-b border-nat-border/30 hover:bg-nat-light-cream/50 transition-colors">
                    <td className="py-4 px-2">
                      <div className="font-bold text-nat-dark text-sm">{h.commodity}</div>
                      <div className="text-[10px] text-nat-sage uppercase">#{h.id.substring(0, 8)}</div>
                    </td>
                    <td className="py-4 px-2">
                      <div className="text-nat-dark">
                        <span className="text-[10px] text-nat-sage mr-1">Tanam:</span>
                        {h.plantingDate}
                      </div>
                      <div className={`mt-0.5 font-bold ${isNearingHarvest ? 'text-amber-600' : 'text-nat-green'}`}>
                        <span className="text-[10px] text-nat-sage mr-1 font-normal">Est. Panen:</span>
                        {h.expectedHarvestDate}
                      </div>
                    </td>
                    <td className="py-4 px-2">
                      <div className="font-bold">{h.expectedVolume.toLocaleString('id-ID')} Kg</div>
                      <div className="text-[10px] text-nat-sage">@ Rp{h.askingPrice.toLocaleString('id-ID')}</div>
                    </td>
                    <td className="py-4 px-2">
                      <div className="flex flex-col gap-1.5 items-start">
                        <span
                          className={`inline-block px-2 py-1 rounded text-[9px] font-bold tracking-wider uppercase border ${
                            h.status === 'ACTIVE' ? 'bg-amber-50 text-amber-700 border-amber-200' :
                            h.status === 'HARVESTED' ? 'bg-nat-light-cream text-nat-green border-nat-green/30' :
                    ...
                    <td className="py-4 px-2 text-right">
                      {h.status === 'ACTIVE' ? (
                        ...
                        <span className="text-[10px] text-nat-sage italic font-medium">Batch Created</span>
                      )}
                    </td>
                  </tr>
                );
              ...
              <tr>
                <td colSpan={5} className="py-8 text-center text-nat-sage italic">
